"""
Gera MIPS ASM de uma AST com anotações

Notas:
Resultados são sempre esperados em $t0
"""
...
def codeGen(tree: Node, file: str):
    if tree.errors:
        print("Erros encontrador, compilação não concluída!")
        return
...
def makeASM(node: Node, asm: ASM):
    if node.nodeType == Node.PROGRAM:
        ...
        raise UnimplementedError(f"Ainda não implementado: {node.nodeType}")
