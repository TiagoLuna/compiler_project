...
class Table:
    def __init__(self, node, exhaustive=True):
        ...
        if node.nodeType == Node.PROGRAM:
            ...
            raise Exception(f"ERROR: Tipo de nó não suportado para geração de símbolo: {node.nodeType}")
...
def scanTree(stack: ScopeStack, node: Node):
    ...
    if node.nodeType is Node.PROGRAM:
        ...
        if not hasattr(node, 'main'):
            error(node, "O programa NÃO contém a função main")
        ...
        if node.datatype is Node.TYPE_INT and not has_return:
            print(f"WARNING: Função {node.name} não deve retornar INT")
...
def assertDeclarationCorrect(node: Node):
    if node.nodeType == Node.FUNC:
        ...
        if node.datatype == Node.TYPE_VOID:
            error(node, "Não é possível declarar variável de tipo vazio")
...
def assertReturnCorrect(stack: ScopeStack, node: Node):
    ...
    if funcScope is None:
        raise Exception("Entrada inválida: Retornar função externa!")
    expected = funcScope.datatype

    if expected is Node.TYPE_VOID and node.value is not None:
        error(node, f"Função {funcScope.name} deveria retornar tipo {expected}")
...
def assertAssignCorrect(stack: ScopeStack, node: Node):
    ...
    if entry is None:
        error(node, f"Símbolo não encontrado: {to.name}")
        return None
    # Check l-value
    if entry.varcard is Node.VAR_ARRAY and to.varcard is Node.VAR_SINGLE:
        error(node.to, f"Não pode atribuir ao ARRAY")
        return None
...
def assertCallCorrect(stack: ScopeStack, node: Node):
    ...
    if entry is None:
        error(node, f"Símbolo não encontrado: {node.name}")
        return None
    if entry.nodeType is not Node.FUNC:
        error(node, f"Símbolo referenciado (definido como {entry.nodeType}) como uma função")
        return None
    if len(node.args) != len(entry.params):
        error(node, f"Função {node.name} esperava {len(entry.params)} param(s), encontrou: {len(node.args)}")
    
    if entry.name == 'main':
        error(node, 'Função "main" não pode ser chamada recursivamente.')
...
def assertReferenceCorrect(stack: ScopeStack, node: Node):
    ...
    if entry.nodeType is not Node.DECLARATION and entry.nodeType is not Node.PARAM:
        error(node, f"Símbolo referenciado (definido como {entry.nodeType}) como varíavel")
        return None
...
def assertLiteralCorrect(node: Node):
    if node.value < -32768 or node.value > 32767:
        error(node, "Literal excede 16 bits")
...
def assertType(stack: ScopeStack, node, target_type, target_card, vervose: str = None):
    ...
    if found != target_type or fcard != target_card:
        ...
        error(node, f"Tipo incompatível {vervose}- esperava: {target_type} {target_card}, encontrou: {found} {fcard}")
...
def calcType(stack: ScopeStack, node):
    ...
    if node.nodeType is Node.VAR:
        ...
        if entry.varcard is Node.VAR_ARRAY:
            ...
            if node.varcard is Node.VAR_ARRAY:
                error(node, "Ler varíavel como array")
                return None
            else:
                cardinality = Node.VAR_SINGLE
    ...
    if node.nodeType is Node.COMP or node.nodeType is Node.SIGM or node.nodeType is Node.PROD:
        ...
        if lc != Node.VAR_SINGLE:
            error(node.left, f"Operador da esquerda não pode ser ARRAY")
            return None
        if rc != Node.VAR_SINGLE:
            error(node.right, f"Operador da direita não pode ser ARRAY")
            return None

        if lt == rt:
            return lt, lc
        else:
            error(node, f"Tipo imcompatível {lt} e {rt}")
            return None
